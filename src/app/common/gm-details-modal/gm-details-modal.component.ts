...
export class GmDetailsModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.backend
      ...
      .subscribe({
        next: (records) => {
          const gmRecords = records
            .filter(r => r.userId === this.gm.userId);

          const systemIdsOrdered = gmRecords.map(r => r.systemId);

          this.backend.getByIds<IRPGSystem>('systems', systemIdsOrdered).subscribe({
            next: (systemsData) => {
            ...
            error: (err) => console.error('Błąd podczas ładowania systemów:', err),
