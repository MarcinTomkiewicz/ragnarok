...
export class GmSchedulingService {
  ...
  upsertAvailability(entry: IAvailabilitySlot): Observable<IAvailabilitySlot> {
    const payload: IAvailabilitySlot = { ...entry, userId: entry.userId ?? (entry as any).gmId, workType: 'gm' };
    return this.availability.upsert(payload);
  }

  upsertMany(entries: IAvailabilitySlot[]): Observable<IAvailabilitySlot[]> {
    const normalized = entries.map(e => ({ ...e, userId: e.userId ?? (e as any).gmId, workType: 'gm' as const }));
  ...
  private isAvailableDuringTimeRange(gmId: string, date: string, startHour: number, duration: number): Observable<boolean> {
    const endHour = startHour + duration;
    return this.availability.getOneForDay(gmId, date, 'gm').pipe(
      map((availability) => !!availability && availability.fromHour <= startHour && availability.toHour >= endHour)
    );
  }

  private isGmBusyAtSlot(gmId: string, date: string, startHour: number, duration: number): Observable<boolean> {
  ...
  getAvailableGmsForSystem(systemId: string, date: string, startHour: number, duration: number): Observable<IGmData[]> {
      ...
      switchMap((avail) => {
        ...
        return this.reservations.getReservationsForGm(gmId, date).pipe(
          map((reservations) => {
            ...
            for (const b of busy) {
              if (b.from > cursor) free.push({ from: cursor, to: Math.min(b.from, avail.toHour) });
    ...
    return combineLatest(
      candidates.map((c) =>
        this.getGmFreeRanges(gmId, c.date).pipe(
          map((free) => {
            const ranked = this.chunkFreeRangesToStarts(free, duration).sort(cmp);
            const slots = ranked.map((h) => ({ date: c.date, startHour: h, duration }))
  ...
  moreSlots(
    ...
    if (mode === GmSlotsMode.next) {
      ...
      return from(days).pipe(
        ...
        map((hit) => {
          if (!hit) return [];
          const starts = this.chunkFreeRangesToStarts(hit.free, duration).sort(cmp);
          return starts.slice(0, 8).map((h) => ({ date: hit.date, startHour: h, duration }))
                       .sort((a, b) => a.startHour - b.startHour);
        })
      );
    }
    ...
    if (mode === GmSlotsMode.weekend) {
      ...
      const sat = new Date(base); sat.setDate(base.getDate() + deltaToNextSat);
    ...
    return combineLatest(
      days.map((date) =>
        this.getGmFreeRanges(gmId, date).pipe(
          map((free) => {
            const starts = this.chunkFreeRangesToStarts(free, duration).sort(cmp);
            return starts.slice(0, MAX_PER_DAY)
      ...
      map((perDay) => perDay.flat().sort((a, b) => a.date.localeCompare(b.date) || a.startHour - b.startHour))
    );
  }

  private byPreferredHour(preferredStartHour?: number) {
    return (a: number, b: number) => this.compareStarts(a, b, preferredStartHour);
  }
