...
import {
  messageForError,
  normalizeErrorCode,
} from '../../../core/enums/errors';
import { isHostSignupConflict } from '../../../core/errors/host-conflicts.error';
import { IEventHostCreate } from '../../../core/interfaces/i-event-host';
...
export class HostSignupFormComponent {
  ...
  private loadTakenRooms$(): Observable<Set<string>> {
    ...
    if (!ev || !date || !ev.rooms?.length) return of(new Set<string>());

    this.takenLoadingSig.set(true);
    return this.hosts
      .listTakenRooms(ev.id, date)
      .pipe(finalize(() => this.takenLoadingSig.set(false)));
  }
  ...
  submit() {
    ...
    const payload: IEventHostCreate = {
      ...
      hostUserId: user.id,
      role: ev.hostSignup, // <— bez mapowania
      title: v.title,
    ...
    this.hosts
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          if (!tpl) return;
          const header = isHostSignupConflict(err)
            ? err.message
            : messageForError(normalizeErrorCode(err));
          this.toast.show({
            template: tpl,
