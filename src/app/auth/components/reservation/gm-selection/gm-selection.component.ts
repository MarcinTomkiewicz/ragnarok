...
export class GmSelectionComponent {
  ...
  constructor() {
    this.reservationService.getAllSystems()
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(s => {
        this.allSystems.set(s);
        if (!this.isPartyGmLocked()) this.systems.set(s);
      });

    // Wejście/wyjście z trybu „party-GM”
    toObservable(this.isPartyGmLocked)
      .pipe(distinctUntilChanged(), takeUntilDestroyed(this.destroyRef))
      .subscribe(locked => locked ? this.enterLockedMode() : this.exitLockedMode());

    // Zmiana systemu
    this.form.get('systemId')!.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef))
  ...
  private enterLockedMode() {
    ...
    this.partyService.getPartyOwnerData(gmId)
      .pipe(takeUntilDestroyed(this.destroyRef))
    ...
    this.gmService.getSystemsForGm(gmId)
      ...
      .subscribe(list => {
        ...
        // dobranie właściwego systemu i NATYCHMIASTOWE sprawdzenie dostępności
        const current = this.form.value.systemId as string | null;
        const allowed = new Set(list.map(s => s.id));
        const next = current && allowed.has(current) ? current : (list[0]?.id ?? null);
  ...
  private exitLockedMode() {
    ...
    this.systems.set(this.allSystems());
  ...
  private updateLockedGmAvailability() {
    ...
    this.gmService.getAvailableGmsForSystem(systemId, date, startHour, duration)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(list => {
  ...
  showOnlyAvailableNow() {
    ...
    this.gmService.getAllGmsForTimeRange(date, startHour, duration)
      .pipe(takeUntilDestroyed(this.destroyRef))
  ...
  readonly gmsForSystemAll = toSignal(
    this.form.get('systemId')!.valueChanges.pipe(
      switchMap((systemId: string | null) =>
        systemId ? this.gmService.getGmsForSystem(systemId) : of([] as IGmData[])
      )
    ),
