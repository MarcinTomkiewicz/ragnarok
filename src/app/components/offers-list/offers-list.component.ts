...
export class OffersListComponent implements OnInit {
  ...
  loadInitialData(): void {
    ...
    const offers$ = this.backendService.getAll<Offer>(
      'offers', 
      'title', 
      'asc', 
    );
  
    forkJoin({ categories: categories$, subcategories: subcategories$, offers: offers$ }).subscribe({
      next: ({ categories, subcategories, offers }) => {
        this.categories = categories;
        this.subcategories = subcategories;
        this.offersList = offers;
      },
      error: (err) => console.error('Błąd podczas pobierania danych:', err),
      complete: () => console.log('Wszystkie dane zostały pobrane'),
    });
  }
